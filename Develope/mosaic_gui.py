...
class MosaicWorker(QtCore.QThread):
    ...
    def run(self):
        try:
            ...
            # Write Fiji Grid/Collection stitching helper (works with tile_{i}.tif)
            try:
                max_c = max(p[2] for p in self.positions) if self.positions else -1
                max_r = max(p[3] for p in self.positions) if self.positions else -1
                grid_x = max_c + 1
                grid_y = max_r + 1
                settings_path = os.path.join(self.output_dir, "AcqSettings_for_Fiji_GridStitching.txt")
                with open(settings_path, "w", encoding="utf-8") as sf:
                    sf.write("Fiji Grid/Collection stitching recommended settings for this dataset\n")
                    sf.write("Type: Grid: row-by-row\n")
                    sf.write("Order: Grid: snake by rows\n")
                    sf.write("Direction: Right & Down\n")
                    sf.write(f"Grid size x (columns): {grid_x}\n")
                    sf.write(f"Grid size y (rows): {grid_y}\n")
                    sf.write("First file index i: 0\n")
                    sf.write("File names for tiles: tile_{i}.tif\n")
                    sf.write("Tile overlap [%]: 10-20 (match acquisition)\n")
                    sf.write("Fusion: Linear blending\n")
                    sf.write("Compute overlap: checked\n")
            except Exception:
                pass
