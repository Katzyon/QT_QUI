...
class Protocol:
    ...
    def __init__(self, Brigde):
        ...
        self.stagePeriod = float() # time of one sequence cycle in ms
    ...
    def calc_interMaskInterval(self):
        ...
        self.interMaskInterval = (self.stagePeriod / len(self.sequence)) - self.onTime # ms
        
        # show a warning window if interMaskInterval is negative
        if self.interMaskInterval < 0:
            msg = QMessageBox()
            msg.setIcon(QMessageBox.Critical)
            msg.setWindowTitle("Timing error")
            msg.setText("interMaskInterval < 0. Change onTime, Frequency or Group number")
            msg.exec()
        ...
        print("stagePeriod:", self.stagePeriod, "groupsNumber:", len(self.sequence), "onTime:", self.onTime ,"interMaskInterval:", self.interMaskInterval)
        ...
        self.cycleTime = self.stagePeriod/1000 # one cycle of a stage in seconds
        self.sequenceRepeats = round(self.stimTime*60 / self.cycleTime) # number of protocol repeats for running the protocol
        print("stimTime sec:", self.stimTime*60, "cycleTime:", self.cycleTime, "divide =", self.stimTime*60 / self.cycleTime) 
       

        self.imageRepeats = self.sequenceRepeats * self.groupsNumber

        print("Stage period:", self.stagePeriod, "ms ,",  " cycleTime:", self.cycleTime, "sec")
        print("protocol time", self.stimTime*60, "sec", "calculated running time:", self.imageRepeats*(self.interMaskInterval+ self.onTime)/1000, "sec")
