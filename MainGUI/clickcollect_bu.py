...
class ClickCollector(QWidget):
    ...
    def mousePressEvent(self, event):
        ...
        core.set_slm_image(dmd_name, transformed_img) # load the image to the SLM
        # add a small delay to ensure the SLM image is set before displaying it
        time.sleep(0.01)
        core.display_slm_image(dmd_name) # display the image on the SLM (position the mirrors)
        self.arduino_comm.send_message([1], 100, self.light_click_ms_time) # trigger the light (image out)
