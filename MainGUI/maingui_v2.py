...
class MainGui(QMainWindow, Ui_MainGui): # 
    def __init__(self):
        ...
        # Integration: last ROI-based DMD mask from SimpleStimWindow
        self.roi_dmd_mask = None
        self.roi_dmd_mask_path = None
        self.simple_dmd_window = None
    ...
    def detect_Cells(self): # called by detectCells button (Detect cells)
        ...
        self.imageview.setImage(np.fliplr(self.averageImage).copy())
...
# open protocols window and get info from user
    ...
    def _define_subdirectories(self):
        ...
        self.ROI_dir = os.path.join(base, "ROI")
        self.roi_dmd_mask_path = os.path.join(self.ROI_dir, "roi_mask.bmp")
    ...
    def connect_buttons(self):
        ...
        # Add a menu action to open the Simple DMD Stim window (ROI → DMD mask saver)
        if SimpleStimWindow is not None:
            try:
                tools_menu = None
                for act in self.menuBar().actions():
                    if act.text().replace('&', '').strip().lower() == 'tools':
                        tools_menu = act.menu()
                        break
                if tools_menu is None:
                    tools_menu = self.menuBar().addMenu("&Tools")

                self.open_simple_dmd_stim_action = tools_menu.addAction("Simple DMD Stim (ROI mask)")
                self.open_simple_dmd_stim_action.triggered.connect(self.open_simple_dmd_stim)
            except Exception as e:
                print(f"Warning: could not add Tools->Simple DMD Stim menu action: {e}")

        
    def open_simple_dmd_stim(self):
        """Open (or focus) the Simple DMD ROI stim window.

        - Shares Core/Camera/Arduino handles with MainGUI (no double-connect).
        - Saves the last ROI-derived DMD mask into a protocol-safe folder when available:
            <culture>/Protocols/Protocol_<n>/roi_mask.bmp (+ meta)
        Falls back to self.DMD_dir only if protocol dir is unavailable.
        - Updates self.roi_dmd_mask / self.roi_dmd_mask_path when saved.
        """
        if SimpleStimWindow is None:
            self.show_error_message("Simple DMD", "simple_dmd_stim_gui.py could not be imported.")
            return
        if self.core is None or self.camera is None:
            self.show_error_message("Simple DMD", "Core/Camera not initialized.")
            return

        # Reuse existing window if possible
        if self.simple_dmd_window is not None and self.simple_dmd_window.isVisible():
            self.simple_dmd_window.raise_()
            self.simple_dmd_window.activateWindow()
            return

        affine = getattr(self, "affine_transform", None)
        if affine is None:
            affine = getattr(self, "affine_trans_old", None)

        # Prefer protocol-safe folder
        save_dir = getattr(self, "ROI_dir", None)
        if not save_dir:
            self.show_error_message("Simple DMD", "ROI_dir is not defined. Create/load an experiment first.")
            return

        os.makedirs(save_dir, exist_ok=True)

        self.simple_dmd_window = SimpleStimWindow(
            host_gui=self,
            save_dir=save_dir,
            core=self.core,
            camera=self.camera,
    ...
    def _on_simple_dmd_mask_saved(self, bmp_path: str):
        self.roi_dmd_mask_path = bmp_path
        try:
            img = cv2.imread(bmp_path, cv2.IMREAD_GRAYSCALE)
            self.roi_dmd_mask = img
        except Exception:
            pass
