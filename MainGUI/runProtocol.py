...
class ProtocolRunner(QThread):
    ...
    def __init__(self, gui, DMDArray=None, parent=None):
        ...
        print("number of stages:", len(self.stages))
        ...
        print("DMDArray num stages:", len(self.DMDArray), 
                "num images", self.DMDArray[0].size()) # "image len", self.DMDArray[0][0].size()
                # Get the first ArrayList
        first_array_list = self.DMDArray[0] # get the first Java ArrayList
        ...
        self.rand_vector = [] # list of randomized group numbers
        #self.running = True
...
# if you need to run a distribution of images with different probabilities we need to create:
    ...
    def run(self):
        ...
        self.times = 0
        # iterate over the stages in the protocol
        for index, row in enumerate(self.stages):
            ...
            javaSequence = self.DMDArray[index]  # self.DMDArray.append(javaarray)list of Java arrayLists, each arrayList is a sequence of (vectorizred) images to be displayed on the DMD
            ...
            for i in range(self.stages[index].sequenceRepeats): # need to randomize the sequence every sequence repeat
                if self.stop_event.is_set():
                        print("Aborting the run")
                        break
                ...
                #self.core.setSLMExposure(self.dmd_name, 500) # set the exposure time for the DMD in ms

                # TRANSFER THE SEQUNCE INTERGERS TO ARDUINO and RUN THE PROTOCOL (ARDUINO CONTROLS THE POLYGON, light source and MaxOne digipins encoding.)
                # 


                # DISPLAY THE IMAGES - iterate over the repeats of the stage
                #for i in range(self.stages[index].imageRepeats): #prot.stages[0].imageRepeats
                for i in range(self.stages[index].groupsNumber): #prot.stages[0].imageRepeats
                    ...
                    # Send to Arduino the current group number (it will trigger the polygon and send the encoded number to MaxOne) - int_to_digital.ino
                    #self.core.sendToArduino(self.stages[index].sequence[i]) # send the group number to the Arduino
                    # How to set the image display time in the DMD?

                    # Wait image display feedback from the Arduino
                    

            # display black image at the end of the stage ???   
