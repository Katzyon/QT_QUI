...
class SimpleStimWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        grid.addWidget(self.active_led, 5, 0); grid.addWidget(self.active_lbl, 5, 1); grid.addWidget(self.stim_progress, 5, 2, 1, 4)
        # Row 6: Status
        self.status_lbl=QLabel(""); grid.addWidget(self.status_lbl,6,0,1,6)

        # Row 7: Arduino
        self.port_edit=QLineEdit("COM13"); self.connect_btn=QPushButton("Connect Arduino")
        grid.addWidget(QLabel("Arduino Port:"),7,0); grid.addWidget(self.port_edit,7,1); grid.addWidget(self.connect_btn,7,2)
